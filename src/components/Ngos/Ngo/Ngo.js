...
const Ngo = (props) => {
    ...
    let ngo=ngoUse.ngo
    useEffect(()=>{
		axios.get('https://hooks-practce.firebaseio.com/donations.json').then(response=>{
			let ngoData = [...ngo];
			var sortable={
				"NGO1":[],
				"NGO2":[],
			...
			for(let key in response.data){
				...
				sortable[response.data[key].ngo].push([response.data[key].name, +response.data[key].orderData.amount])
			}
			...
			for(let key in sortable){
				...
				ngoData[index].sortedDonations.splice(5,ngoData[index].sortedDonations.length-1);
            }
    ...
    const TopContributors=props.info.sortedDonations.map((ele,index)=>{
        return(
        <ListGroupItem key={index}>{ele[0]}: ₹{ele[1]}</ListGroupItem>
        );
    ...
    return (
        <div>
            <Row>
                ...
                <Card.Header className={classes.CardHeader}>{props.info.name}</Card.Header>
                    <Card.Body>
